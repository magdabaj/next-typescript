...
export const processQuestionsData = async (questions: string[], question:string) => {
 const   {
   bagOfWords,
   allWords,
   wordReference
 } = getVocabularyList(questions)

const { outputs, maxSentenceLength, vectors } = createTokenizedVector(questions, wordReference)
...
// model.add(tf.layers.simpleRNN( { units: 32 } ) );
  ...
  let qVec: number[] = [];
  let words = question.replace(/[^a-z ]/gi, "").toLowerCase().split( " " ).filter( x => !!x );
