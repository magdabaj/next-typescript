...
export const UserProvider: React.FC = ({ children }) => {
  ...
  const { data, error } = useSWR(
    `https://accounts.spotify.com/authorize?client_id=${process.env.CLIENT_ID}&response_type=code$redirect_uri=http://localhost:3000&scope=""`
  )
  const getToken = async () => {
    ...
    const response = await axios.get(
      `https://accounts.spotify.com/authorize?client_id=cf76b9c294774ee2810c399056456945&response_type=code$redirect_uri=http://localhost:3000/callback&scope=`,
